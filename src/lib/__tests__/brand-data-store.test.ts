...
test('writes contact payload with metadata and backup', async () => {
  ...
  try {
    ...
    const payload = writeBrandPayload<TestBrandRecord>('contact', [{ brandName: 'New' }], '管理员', root)
    ...
    const read = readBrandPayload<TestBrandRecord>('contact', root)
