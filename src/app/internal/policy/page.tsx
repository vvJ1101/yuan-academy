...
import { Copy, Edit3, Save, Plus, Trash2, Loader2, X, Download, ChevronDown, Upload } from 'lucide-react'
...
export default function PolicyPage() {
  ...
  const canEdit = can(permissions, 'brandOrdering.edit')
  const canUpload = can(permissions, 'brandOrdering.upload') || can(permissions, 'menu.policyUpload')
  const canExport = can(permissions, 'brandOrdering.export')
  const canDelete = can(permissions, 'brandOrdering.delete')
  const canDeleteAll = can(permissions, 'brandOrdering.deleteAll')

  useEffect(() => {
    fetch('/api/auth/me').then(r=>r.json()).then(u=>{ if(Array.isArray(u?.permissions))setPermissions(u.permissions) }).catch((err: any) => console.warn("[SilentError]", err))
  }, [])
  useEffect(() => {
  ...
  async function saveCard(){
    if(!canEdit){ setSaveMsg('你没有编辑订货政策的权限'); return }
  ...
  function addBrand(){ if(!canEdit){ setSaveMsg('你没有添加订货政策的权限'); return } const ni=items.length; setItems(p=>[...p,{...EMPTY}]); setTimeout(()=>{ setEditingIdx(ni);setEditDraft({...EMPTY});setExpanded(p=>{const n=new Set(p);n.add(ni);return n});
    const el=document.querySelector('[data-idx="'+ni+'"] input'); if(el)(el as HTMLElement).focus() },50) }

 async function removeBrand(idx:number){ if(!canDelete){ setSaveMsg('你没有删除订货政策的权限'); return } if(!confirm('确定删除?'))return; setEditingIdx(null);setEditDraft(null); const upd=items.filter((_,i)=>i!==idx); setSaving(true)
  ...
  async function removeAllBrands(){ if(!canDeleteAll){ setSaveMsg('你没有删除全部订货政策的权限'); return } if(!confirm(`确定删除全部 ${items.length} 个品牌？此操作不可恢复。`))return; setSaving(true)
  ...
  return (<div className="p-4 md:p-8 lg:p-10 max-w-4xl overflow-x-hidden">
    ...
    <BrandTabs active="ordering" />
    <div className="flex items-center gap-1.5 flex-wrap">{canEdit&&<button onClick={addBrand} className="min-h-[44px] px-3 py-1.5 text-[0.75rem] rounded-lg border bg-white text-neutral-700 border-neutral-300 hover:bg-neutral-50 flex items-center gap-1"><Plus size={13}/> 添加</button>}{canUpload&&<Link href="/internal/policy-upload" className="min-h-[44px] px-3 py-1.5 text-[0.75rem] rounded-lg border bg-white text-neutral-700 border-neutral-300 hover:bg-neutral-50 flex items-center gap-1 no-underline"><Upload size={13}/> 上传更新</Link>}{canExport&&<button onClick={exportExcel} className="min-h-[44px] px-3 py-1.5 text-[0.75rem] rounded-lg border bg-white text-neutral-700 border-neutral-300 hover:bg-neutral-50 flex items-center gap-1"><Download size={13}/> 导出</button>}{canDeleteAll&&<button onClick={removeAllBrands} disabled={saving||items.length===0} className="min-h-[44px] px-3 py-1.5 text-[0.75rem] rounded-lg border bg-white text-red-500 border-red-200 hover:bg-red-50 flex items-center gap-1 disabled:opacity-50"><Trash2 size={13}/> 全部删除</button>}<button onClick={()=>{if(filtered.length===0)return; copyText(filtered.map(p=>fmtPolicy(p)).join('\n\n---\n\n')).then(()=>{setCopyAllId(true);setTimeout(()=>setCopyAllId(false),2000)})}} className={`min-h-[44px] px-3 py-1.5 text-[0.75rem] rounded-lg border font-medium ${copyAllId?'bg-emerald-50 text-emerald-700 border-emerald-300':'bg-[#2563EB] text-white border-[#2563EB] hover:bg-blue-600'}`}>{copyAllId?'已复制全部':'复制全部'}</button><button onClick={toggleAll} className="min-h-[44px] text-[0.75rem] text-neutral-500 hover:text-neutral-900">{expandAll?'收起全部':'展开全部'}</button></div></div>
