...
const adminCards = [
  { href: '/internal/admin/org', icon: Building2, title: '组织架构', desc: '管理公司、部门结构与层级关系', color: 'bg-indigo-50 border-indigo-100', permission: 'menu.admin.org' },
  { href: '/internal/admin/folders', icon: Folder, title: '文件夹管理', desc: '管理知识库目录结构与访问权限', color: 'bg-slate-50 border-slate-200', permission: 'menu.admin.folders' },
  ...
  { href: '/internal/admin/settings', icon: Wrench, title: '系统设置', desc: '基础配置与维护', color: 'bg-gray-50 border-gray-200', permission: 'menu.admin.settings' },
  { href: '/internal/admin/audit-log', icon: Clock, title: '审计日志', desc: '用户操作与系统变更记录', color: 'bg-sky-50 border-sky-100', permission: 'menu.admin.audit' },
]

function can(permissions: string[], key: string) {
  return permissions.includes('*') || permissions.includes(key) || permissions.some(p => p.startsWith(`${key}.`))
}
...
export default function AdminPage() {
  ...
  const visibleCards = adminCards.filter(card => can(permissions, card.permission))
  ...
  return (
    <div className="max-w-[800px] mx-auto px-5 md:px-8 py-8">
      ...
      {visibleCards.length === 0 && (
        <div className="rounded-xl border border-dashed border-neutral-200 bg-white px-5 py-10 text-center">
          <p className="text-[0.85rem] text-neutral-500">暂无可管理的模块</p>
        </div>
      )}
    </div>
  )
