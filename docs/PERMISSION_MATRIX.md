...
- 优先按缩进树找权限：第一层是模块（知识中心、品牌资料、智能应用、系统管理），第二层是页面，第三层是按钮或数据动作。
- 搜索权限时只显示命中的层级路径；“选择搜索结果”只批量勾选当前可见结果，不会勾选隐藏权限。
- 市场部建议使用“市场部模板”，只包含品牌对接市场字段和订货政策查看。
- 商品部建议使用“商品部模板”，包含品牌对接完整维护权限和订货政策查看。
- 品牌部建议使用“品牌部模板”，包含订货政策维护权限。
- 保存前如果出现高危权限提示，需要复核是否真的需要删除、完整导出、系统设置或角色分配权限。
- 模板只是批量勾选建议，不是写死部门规则；最终权限仍以管理员保存的勾选结果为准。
